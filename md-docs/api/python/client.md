...
```python
.create_detection_event_rule(
   name: str, task_id: str, detection_event_type: DetectionEventType,
...
Severity can be None for detection events WARNING_ON and WARNING_OFF, however, it
is mandatory for DRIFT_ON, DRIFT_OFF.
...
```py
# Create a rule that matches any drift on event and send a notification on MQTT channel
...
# Create a rule that matches only drift events on input plain monitoring target on the all population
client.create_detection_event_rule(
    name='Drift on rule',
    task_id=task_id,
    detection_event_type=DetectionEventType.DRIFT_ON,
...
```python
.update_detection_event_rule(
   rule_id: str, detection_event_type: DetectionEventType,
   actions: list[DetectionEventAction], name: str,
   severity: (DetectionEventSeverity|None) = None,
   monitoring_targets: (list[MonitoringTarget]|None) = None,
   monitoring_metrics: (dict[MonitoringTarget,
   list[MonitoringMetric]]|None) = None, segment_ids: (list[str|None]|None) = None
...
* **severity**  : the level of severity of the detection event
    that this rule should respond to. None means any
    severity is matched. None means ANY value.
* **actions**  : the list of actions to execute, in order,
    when the conditions of the rule are matched.
* **segment_ids**  : the list of segment ids on which the event should take
    place to trigger the rule. If the element in the list is None,
    the rule will be triggered on events related to the whole
    population. None means ANY value.
...
```python
.wait_job_completion(
   job_id: str, max_wait_timeout: int = 7200
)
```
